I was not sure how y'all preferred sumologic would communicate with lambda so I assumed a webhook set up that will trigger a lambda through an API gateway. Sumo Logic can call an HTTPS webhook. Lambda is not directly callable over HTTPS unless you put something in front of it. This set up, I think, gurantees comms from a sumologic webhook to lambda. I also have to create these resources before I can test the lambda so a bit constrained with the testing part.
